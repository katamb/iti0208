<template>
    <div>
      ...
      <form id="post-form" @submit.prevent="processForm">
        ...
        <div class="error" v-if="errors.has('title')">{{errors.first('topic')}}</div>

        Title:<br>
        <input type="text" name="title" placeholder="Title" v-model="title"
               v-validate="{ required: true, min: 3 }"><br>
        ...
        <input type="text" name="reward_description" placeholder="Reward Description" v-model="reward_description"><br>
        <div class="error" v-if="errors.has('reward_description')">{{errors.first('reward_description')}}</div>

        File:<br>
        <input id="singleFileUploadInput" type="file" name="file" class="file-input" @change="loadTextFromFile"/><br>
